...
async function takeSnapshot() {
  const modelViewer = document.getElementById('modelViewer');
  try {
    const blob = await modelViewer.toBlob({ idealAspect: true, quality: 1 });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = (document.getElementById('viewerProjectName').textContent || 'Model') + '_3D.png';
    a.click();
    URL.revokeObjectURL(url);
    showToast('Fotoğraf indirildi! 📸', 'success');
  } catch (err) {
    showToast('Fotoğraf işlemi desteklenmiyor.', 'error');
  }
}

function toggleEnvDropdown() {
  document.getElementById('envDropdown').classList.toggle('active');
}

function changeEnv(envId) {
  const modelViewer = document.getElementById('modelViewer');
  if (!envId) {
    modelViewer.environmentImage = '';
    modelViewer.exposure = 0.4; // Gece
  } else {
    modelViewer.environmentImage = envId;
    modelViewer.exposure = 1;
  }
  document.getElementById('envDropdown').classList.remove('active');
  showToast('Ortam değiştirildi.', 'info', 2000);
}

// Close dropdown if clicked outside
document.addEventListener('click', (e) => {
  const btn = document.getElementById('envBtn');
  const drop = document.getElementById('envDropdown');
